...
exports.getShops = (req, res, next) => {
  Seller.find()
    ...
    .sort({ createdAt: -1 })
    .then((sellers) => {
      const sellersFinal = sellers.filter((shop) => {
        return shop.account.isVerified === true;
      });
      res.status(200).json({
        shops: sellersFinal,
...
exports.postCart = (req, res, next) => {
  ...
  Item.findById(itemId)
    ...
    .then((result) => {
      res
        .status(200)
        .json({ message: "Item successfully added to cart:", result: result });
    })
    .catch((err) => {
      if (!err.statusCode) err.statusCode = 500;
      next(err);
...
exports.getLoggedInUser = (req, res, next) => {
  ...
  let decodedToken;
  try {
    decodedToken = jwt.verify(token, "supersecretkey-multivander");
  } catch (err) {
    err.statusCode = 500;
    throw err;
  }
    ...
    .then((result) => {
      ...
      for (let [seller, cartItem] of Object.entries(sellers)) {
        Seller.findById(seller).then((seller) => {
          ...
          order.save();
          for (const clientId of Object.keys(opp.clients)) {
